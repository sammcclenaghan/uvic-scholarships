...
/**
 * Graduate scholarship data extraction script.
 *
 * 1. Reads the cached graduate API response from data/raw-graduate-api-response.json
 * 2. Parses and cleans the raw HTML data (graduate data is grouped by department)
 * 3. Batches scholarships and sends them to an LLM (via opencode CLI)
 *    to extract structured application requirements
 * 4. Writes enriched data to data/graduate-scholarships.json
 *    and filter metadata to data/graduate-metadata.json
 *
 * Supports resuming - if data/graduate-scholarships.json already exists with
 * some enriched entries, it will skip those and continue from where it left off.
 *
 * Usage:
 *   node scripts/extract-graduate.mjs
 */
...
const OUTPUT_FILE = join(DATA_DIR, "graduate-scholarships.json");
const METADATA_FILE = join(DATA_DIR, "graduate-metadata.json");
const RAW_CACHE_FILE = join(DATA_DIR, "raw-graduate-api-response.json");

const API_URL = "https://webfilters.uvic.ca/api/fgsawards?start=0&end=600";
const BATCH_SIZE = 15;
...
/**
 * Graduate row structure (6 cells):
 *   [0] name/description HTML
 *   [1] deadline text
 *   [2] amount text
 *   [3] timestamp (hidden, ignored)
 *   [4] tags for type (PhD, Masters, External awards, etc.)
 *   [5] tags for department
 */
function parseGraduateScholarship(row, index) {
  ...
  const awardType = row[4]?.tags || [];
  const departments = row[5]?.tags || [];

  // Infer applicationRequired from deadline text
...
function buildPrompt(batch) {
  ...
  return `You are a data extraction assistant. Given the following graduate scholarship descriptions from the University of Victoria, extract structured information for each one.

For EACH scholarship, return a JSON object with these fields:
- "id": the scholarship id (pass through from input)
- "documents": string array of required documents (e.g. ["Letter of reference (400 words max)", "Cover letter (500 words max)", "Research proposal"]). If no specific documents are mentioned, return an empty array [].
- "whereToApply": one of: "UVic Online Tools", "Faculty of Graduate Studies", "Faculty website", "Department", "Automatic - no application needed", or a specific description if mentioned
- "applyUrl": a direct URL if one is mentioned in the description, otherwise null
- "eligibilitySummary": a 1-2 sentence plain-English summary of who qualifies (degree level, program, criteria)
- "financialNeed": boolean, true if financial need is mentioned as a preference or requirement
- "yearRequirement": string like "entering", "continuing", "completing thesis", etc. or null if not specified
...
- These are GRADUATE scholarships - pay attention to whether they target Masters, PhD, or both
...
async function main() {
  console.log("=== UVic Graduate Scholarship Data Extraction ===\n");

  // Step 1: Fetch raw data (or use cache)
  let rawData;
  if (existsSync(RAW_CACHE_FILE)) {
    const cached = JSON.parse(readFileSync(RAW_CACHE_FILE, "utf-8"));
    // Check if cache has all the data (old cache may only have first page)
    let cachedRows = 0;
    for (const group of cached.data) {
      cachedRows += group.data.length;
    }
    if (cachedRows >= (cached.display?.total || 0)) {
      console.log("Using cached API response...");
      rawData = cached;
    } else {
      console.log(
        `Cached response only has ${cachedRows}/${cached.display?.total} rows, re-fetching...`
      );
      rawData = null;
    }
  }
  ...
  if (!rawData) {
    console.log("Fetching graduate scholarships from UVic API...");
  ...
  const totalCount = rawData.display?.total || 0;
  console.log(`Total graduate scholarships (from API): ${totalCount}\n`);

  // Step 2: Flatten grouped data into a single array of rows
  // Graduate data is grouped by department: { label, data: [[row], [row], ...] }
  const allRows = [];
  ...
  console.log(`Flattened ${allRows.length} scholarship rows from ${rawData.data.length} department groups\n`);

  // Step 3: Parse all scholarships
  const scholarships = allRows.map((row, i) => parseGraduateScholarship(row, i));
  ...
  // Step 5: Batch and enrich unenriched scholarships
  ...
  if (unenriched.length === 0) {
    ...
    saveFilterMetadata(rawData, scholarships);
    return;
  }
...
function saveFilterMetadata(rawData, scholarships) {
  // Graduate filters come from rawData.filters (flat array, not nested in .row)
  const filtersData = rawData.filters || [];

  // Map to the same shape as undergrad metadata
  const metadata = {
  ...
  console.log(`Filter metadata saved to ${METADATA_FILE}`);
}
